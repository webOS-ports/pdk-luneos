...
| SIGSEGV shortly after the first GL call, under emulation | llvmpipe's LLVM JIT faulting under `qemu-arm` — LLVM compiles to ARM and qemu then re-translates it, a double JIT. It tracks **LLVM**, not Mesa: Mesa 24.0.7 and Mesa 26.2.0 both crash with LLVM 18, while Mesa 22.3 with LLVM 15 was fine. `pdk-run` therefore defaults to `GALLIUM_DRIVER=softpipe` on the emulated path — correct, and roughly 7x slower. Does not apply on real ARM hardware: no emulation, and freedreno is used instead. |
