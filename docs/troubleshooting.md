...
| `Unable to find target for this triple (no targets are registered)`, then a crash or no GL | LLVM was cross-built without `LLVM_HOST_TRIPLE`, so `llvm-config.h` got `LLVM_NATIVE_ARCH X86` and left `LLVM_NATIVE_TARGET` undefined — `LLVMInitializeNativeTarget()` then registers nothing. Affects **every** cross-built OE target, not just emulated ones. Fixed by `meta-luneos/recipes-devtools/llvm/llvm_%.bbappend`; check `LLVM_NATIVE_ARCH` in the target's `llvm/Config/llvm-config.h`. |
